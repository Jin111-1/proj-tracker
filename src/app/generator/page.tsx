...
export default function GeneratorPage() {
  ...
  const set1Items: QrDataItem[] = Object.entries(mockData.set1).map(
    ([key, value]) => ({
      label: `${key}: ${value}`,
      value: value,
    }),
    ...
    <div className="flex flex-col items-center min-h-screen p-8 bg-gray-50">
      <div className="w-full max-w-4xl p-6 bg-white rounded-lg shadow-md">
        <h1 className="text-2xl font-bold text-center mb-8 text-gray-800">
          สร้าง QR Code จากข้อมูลจำลอง
        </h1>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="flex flex-col h-full">
            <h2 className="text-lg font-semibold mb-4 text-gray-700">
              เลือกข้อมูล (จาก mockData.json)
            </h2>
            ...
            {selectedItem ? (
              <div className="flex flex-col items-center animate-in fade-in zoom-in duration-300">
                ...
                <div className="bg-blue-50 px-6 py-3 rounded-lg border border-blue-100 w-full text-center">
                  ...
                  <p className="text-md text-gray-800 break-all">
                    {selectedItem.value}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-10 text-center">
          <Link
            href="/"
            className="inline-block px-6 py-2 bg-gray-800 text-white font-medium rounded-lg hover:bg-gray-700 transition-colors shadow-sm"
          >
            กลับไปหน้าแรก
          </Link>
        </div>
